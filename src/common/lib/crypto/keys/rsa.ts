...
export class RSAPrivateKey extends PrivateKey {
  ...
  static async new(
    {
      driver = crypto.subtle,
      type = KeyType.RSA_65537,
      modulusLength,
    }: { driver?: SubtleCrypto; type?: KeyType; modulusLength?: number } = {
      driver: crypto.subtle,
      type: KeyType.RSA_65537,
    }
    ...
    const key = (await driver.generateKey(
      {
        ...(getInitializationOptions(type) as RsaHashedKeyGenParams),
        modulusLength,
      },
      true,
  ...
  static async deserialize({
    ...
    const key = await driver.importKey(
      format as any,
      keyData as any,
  ...
  public async sign(
    ...
    return new Uint8Array(
      await this.driver.sign(signingOptions, this.key, payload)
    );
  }
  ...
  public async public(): Promise<RSAPublicKey> {
    ...
    this.publicKey = await RSAPublicKey.deserialize({
      driver: this.driver,
      format: "jwk",
      keyData,
      type: this.type,
    ...
    switch (this.type) {
      case KeyType.RSA_65537: {
        ...
        for (let s of [0, 32, maxSaltSize(this.key)]) {
          if (
            (result = await this.driver.verify(
              {
                ...(getSigningParameters(this.type) as RsaPssParams),
                saltLength: s,
              },
              this.key,
              signature,
